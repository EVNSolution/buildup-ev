...
describe('입력 초점 확대 방지', () => {
  it('🔴 손가락 기기의 입력칸 글꼴은 zoom 을 되돌려도 16px 이상이다', () => {
     ...
     */
    /*
     * **적힌 값이 16px** — 사파리가 초점 확대를 판정하는 기준선이다.
     *
     * 예전엔 18.5px 이었다. `zoom: .88` 때문에 「그려진 값」으로 판정하더라도 16px 을
     * 넘기려던 값인데, 그 크기가 주변 글씨와 너무 벌어져 **입력칸만 혼자 커 보였다**
     * (제보 3회). 본문을 키우는 쪽은 「본문은 작은 게 좋다」로 거절됐다.
     *
     * ⚠️ 그래서 지금은 **「적힌 값」 판정에만 안전하다.** 실기기에서 확대가 되살아나면
     *    18.5px 로 되돌리거나 `zoom` 자체를 걷어내야 한다 — 그 판단은 기록으로 남긴다.
     */
    expect(fs, '적힌 값이 16px 미만이면 사파리가 확대한다').toBeGreaterThanOrEqual(16);
    expect(zoom, 'zoom 이 바뀌면 그려지는 크기도 바뀐다 — 다시 재 봐야 한다').toBe(0.88);
  });
