# buildup-ev EV2 deploy

PEM/SSH 없이 GitHub Actions → SSM으로 EV2에 배포한다.
...
- `EV2_EC2_INSTANCE_ID`
- `EV2_SERVER_NAME`
- `EV2_APP_ENV` — 그대로 SSM SecureString `/buildup-ev/ev2/app-env`에 저장됨
...
BOOTSTRAP_ADMIN_PW=
# RUN_DB_PUSH=1   # ev2 전용 DB일 때만
# RUN_DB_SEED=1   # ev2 전용 DB일 때만. 운영 공유 DB 금지
```
