...
- **만나는 지점 = 특장 장착.** (그 뒤 묶음의 이름은 「출고」 — 내보내기 위한 마지막 과정) 차량 도착 + 특장 제작 완료가 둘 다 되어야 얹을 수 있다.
...
단계마다 「며칠 넘으면 지연」을 임의로 정하지 않는다. 근거 없는 숫자로 빨갛게 칠하면
정작 진짜 늦은 것을 알아볼 수 없다. **우리가 실제로 가진 약속**만 마감으로 쓴다.

| 단계 | 마감이 오는 곳 | 표시 |
|---|---|---|
| `build_done` 특장 제작 완료 | 주문의 **납기일**(특장사가 수락하며 약속) | 「2026-08-10 마감 · 4일 초과」 |
| `inspection_done` 안전검사 완료 | **검사예정일**(특장사가 신청하며 적음) | 〃 |
| 발주 수락 | 발주 후 **7일**(회의 확정) | 수락 대기 목록 맨 위 + 「발주 후 N일째」 |

나머지 단계는 마감이 없다 — 지연이라 부르지 않는다.

두 가지를 함께 지킨다:
- **마감일 당일까지는 지연이 아니다.** 그날 안에 하면 지킨 것이다.
- **아직 열리지 않은 단계는 지연이 아니다.** 선행이 안 끝나 아무도 시작할 수 없는 일이다.
...
- **하루 두 번(아침·저녁), 한 사람에게 한 통.** 여러 건이 밀려 있어도 메일은 하나다 —
  건별로 쪼개서 보내는 순간 아무도 안 본다. 한 통 안에 「받을 것 / 늦은 것 / 오늘 마감」을 묶는다.
...
- **주문 상세 화면은 「단계 중심」으로 다시 짠다** — 위쪽에 두 갈래 진행,
  아래에 지금 할 단계와 증빙 업로드. 특장사가 「다음에 뭘 해야 하나」를 묻는 화면이다.
  지금은 한 화면(617줄)이 사양·서류·하중 세 가지를 겸하고 있어 1단계에서 갈아엎는다.
...
**`order.status`(옛 6단계)를 읽지 않는다.** 그 값은 확정된 적이 없다 — 확정되지 않은 값에서
「여기까지 왔다」를 유추하면 실제로는 안 한 일이 완료로 찍히고, 그 뒤로는 아무도 그게
거짓인지 알 수 없게 된다. 단계 기록은 증빙과 짝을 이루는 사실 기록이다.

다른 곳에서 **확실히 확인되는 것만** 완료로 표시한다.
...
나머지는 **전부 `pending`**. 담당자가 화면에서 실제 상태를 채운다 —
